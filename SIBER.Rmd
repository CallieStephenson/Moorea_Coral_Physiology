...
```{r}
pc1 <- read_csv("data/pc1.csv")


SI_pc <- left_join(SI, pc1)
ellipse_with_pc1 <- ggplot(data = SI_pc, aes(iso1, iso2)) +
  ...
  theme(strip.text = element_text(face = "italic", size=18),
        legend.position = "bottom",
        plot.background = element_rect(fill = "white")) + 
  stat_ellipse(aes(group = interaction(Species, HS),
                   fill = HS,
                   color = HS),
               alpha = 0,
               level = 0.95,
  ...
  stat_ellipse(aes(group = interaction(Species, HS),
               ...
               type = "norm",
               geom = "polygon") +
  scale_color_viridis_c() +  # Continuous scale for points
  scale_fill_manual(values = c("Host" = "#440154FF", "Symbiont" = "#21908DFF")) +
  scale_color_manual(values = c("Host" = "#440154FF", "Symbiont" = "#21908DFF")) +
  scale_shape(labels = c("Host", "Symbiont"))+
  labs(color = " ", fill = " ", shape = " ")  # Update both color and fill legends
