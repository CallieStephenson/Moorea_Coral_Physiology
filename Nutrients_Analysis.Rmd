...
for (variable in predictors) {
  ...
  formula <- as.formula(paste("Percent_Change ~", variable, "+", random_effects))
  ...
  # Extract the beta (fixed-effect coefficient for the predictor)
  beta_value <- fixef(model)[[variable]] 
...
pru_growth_raw_legend <- ggplot(PRU_model_data, aes(x = pc1, y = Percent_Change, 
  ...
  labs(x = "PC1", y = "Buoyant Weight (% Change)", title = " ") +
  scale_color_fish_d(option = "Coryphaena_hippurus", direction = -1) +
  theme_minimal()+
  theme(plot.background = element_rect(fill = "white")) +
...
for (variable in predictors) {
  ...
  # Fit the model
  model <- lmer(formula, data = PRU_model_data)
...
pac_chl_raw <- ggplot(PAC_model_data, aes(x = pc1, y = Chl_ug.cm.2, color = Genotype, shape = Cage_Uncaged)) +
  geom_point() +
  labs(x = "PC1", 
       y = expression(paste("Total Chlorophyll (μg ", cm^-2, ")")),
      title = " ") +
  scale_color_fish_d(option = "Coryphaena_hippurus", direction = -1) +
  theme_minimal()+
  theme(legend.position="none",
...
plot_chl_growth <- ggplot() +
  ...
  labs(
    y = "Buoyant Weight (% Change)",
    x = expression(paste("Total Chlorophyll (μg ", cm^-2, ")"))
  ) +
  scale_color_fish_d(option = "Coryphaena_hippurus", direction = -1)+
  custom_theme+
...
FOR PLOTTING:
```{r}
response_data_plot <- response_data %>% 
  mutate(Genotype_original = Genotype,
...
```{r}
plot_cn_d13c <- ggplot() +
  geom_point(aes(x = δ13C_symb, y = C_N_ratio_symb, color = Genotype, shape = "Symbiont"), data = response_data_plot) +
  geom_point(aes(x = δ13C_host, y = C_N_ratio_host, color = Genotype, shape = "Host"), data = response_data_plot) +
  labs(
