...
pac_chl_MOD <-ggplot() +
  ...
  annotate("text", y = 3.5, x = -4,
         label = paste("p =", round(p_values["pc1"], 2),
                       "\nMarginal R² =", round(marginal_r2, 2),
                       "\nConditional R² =", round(conditional_r2, 2)),
...
```{r}
legend_pac <- ggpubr::get_legend(
  pac_growth_raw_legend +
    guides(
      shape = guide_legend(
        order = 2,
...
legend_pru <- ggpubr::get_legend(
  pru_growth_raw_legend +
    guides(
      shape = guide_legend(title = "Caging Treatment", title.position = "top"),
      color = guide_legend(title = "Parent Colony", title.position = "top")
    ) +
    theme(
