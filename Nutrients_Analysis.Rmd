...
explanatory_variables <- read.csv("data/explanatory_variables_all.csv")
...
### Thank you to Chris Wall for the code for this:
```{r}
# Compute and show the  result
corplot <- ggcorrplot(cor_matrix, hc.order = TRUE, type = "lower",
          lab = TRUE)

ggsave(corplot, filename = "output/Corr_Plot.png", dpi = 600, width = 7, height = 7)
```
...
pca_plot
#ggsave(pca_plot, filename = "output/PCA_Plot.png", dpi = 600, width = 5, height = 4)
```
...
nut_no_seep$pc2 = (pca$scores[,2]) #don't forget to flip it!
```


```{r}
explanatory_variables_with_pc <- nut_no_seep[,c("CowTagID", "pc1", "pc2", pulse_columns)]
...
#write.csv(explanatory_variables_with_pc, "data/explanatory_variables_with_pc1.csv")
```
...
### plot raw
```{r}
PAC_model_data_clean <- PAC_model_data[!is.na(PAC_model_data$Percent_Change), ]
pac_growth_raw_legend <- ggplot(PAC_model_data_clean, aes(x = pc1, y = Percent_Change, 
                                                          #color = Genotype, 
                                                          shape = Cage_Uncaged)) +
  geom_point() +
  labs(
  ...
  scale_shape_manual(
    values = c("A" = 15, "B" = 17, "C" = 16),
    labels = c("A" = "Caged", "B" = "Partially Caged", "C" = "Uncaged")  # Custom shape labels
  )+
  scale_color_fish_d(option = "Coryphaena_hippurus", direction = -1) +
  theme_minimal()+
  theme(plot.background = element_rect(fill = "white"))+
  guides(color = guide_legend(order = 1, title = "Parent Colony"),
         shape = guide_legend(order = 2, title = "Caging Treatment"))

pac_growth_raw <- pac_growth_raw_legend + theme(legend.position = "none")
...
### linear pc2
```{r}
linear_growth_model_pac_pc2 <- lmerTest::lmer(Percent_Change ~ pc2 +
                                  (1|Genotype)+ (1|Cage_Uncaged), data = PAC_model_data)
#check_model(linear_growth_model_pac)
#summary(linear_growth_model_pac)
Anova(linear_growth_model_pac_pc2)
```
### linear plus temp
```{r}
temp_columns <- names(explanatory_variables)[grepl("Temperature", names(all_nut)) & 
                                     !grepl("Low_Tide_Mean",names(all_nut))]

PAC_model_data_temp <- left_join(PAC_model_data, explanatory_variables[,c("Pin_Number", temp_columns)])

linear_growth_model_pac_temp <- lmerTest::lmer(Percent_Change ~ pc1 + (1|sd_Temperature) +
                                  (1|Genotype) + (1|Cage_Uncaged), data = PAC_model_data_temp)

#check_model(linear_growth_model_pac_temp)
summary(linear_growth_model_pac_temp)
anova(linear_growth_model_pac_temp)
```
...
### plot raw
```{r}
pru_growth_raw_legend <- ggplot(PRU_model_data, aes(x = pc1, y = Percent_Change, 
                                                    #color = Genotype, 
                                                    shape = Cage_Uncaged)) +
  geom_point() +
  scale_shape_manual(
  ...
  theme_minimal()+
  theme(plot.background = element_rect(fill = "white")) +
  guides(shape = guide_legend(title = "Caging Treatment"), color = guide_legend(title = "Parent Colony"))

pru_growth_raw <- pru_growth_raw_legend + theme(legend.position = "none")
...
pac_chl_MOD <-ggplot() +
  geom_point(aes(x = pc1, y = Chl_ug.cm.2, 
                 #color = Genotype, 
                 shape = Cage_Uncaged), data = PAC_model_data) +
  labs(x = "PC1", 
       y = expression(paste("Chlorophyll density (μg ", cm^2, ")")),
      title = " ",
    color = "Genotype",
  ...
  theme_minimal() +
  theme(legend.position="none", #you moved this for Figure2
        plot.background = element_rect(fill = "white"))
...
## Porites rus
```{r}
pru_chl_raw <- ggplot(PRU_model_data, aes(x = pc1, y = Chl_ug.cm.2, 
                                          #color = Genotype, 
                                          shape = Cage_Uncaged)) +
  geom_point() +
  labs(x = "PC1", 
...
# Heterotrophy
An SI thing for Chris:  "Plot your C:N vs. d13C to see if there was a strong pattern, plot a regression to it and check that it isn't rapidly increasing etc. Say that you inspected the C:N vs. d13C for patterns, saw none or whatever. Can be an SI thing"

```{r}
plot_cn_d13c <- ggplot() +
  geom_point(aes(x = δ13C_symb, y = C_N_ratio_symb, color = Genotype, shape = "Symbiont"), data = response_data) +
  geom_point(aes(x = δ13C_host, y = C_N_ratio_host, color = Genotype, shape = "Host"), data = response_data) +
  labs(
    x = expression(delta^13*C),
    y = "Carbon to Nitrogen Ratio",
    shape = "Source"
  ) + 
  scale_shape_manual(values = c("Host" = 17, "Symbiont" = 15)) +
  scale_color_fish_d(option = "Coryphaena_hippurus", direction = -1) +
  theme_minimal() +
  theme(
    legend.position = "bottom",
    plot.background = element_rect(fill = "white"),
    strip.text = element_text(face = "italic", size = 14)  # Italic and larger facet labels
  ) +
  facet_wrap(~Species)
...
model_symb <- lmer(C_N_ratio_symb ~ δ13C_symb + (1 | Genotype), data = response_data)
summary(model_symb)
model_host <- lmer(C_N_ratio_host ~ δ13C_host + (1 | Genotype), data = response_data)
summary(model_host)
```
...
pac_13C_raw <- ggplot(PAC_model_data %>% 
  filter(!is.na(Δ13C)), aes(x = pc1, y = Δ13C
                            #color = Genotype
                            )) +
  geom_point(shape = 15) +
  labs(x = "PC1", 
       y = expression(delta^13*C[H - S]),
      title = " ") +
  scale_color_fish_d(option = "Coryphaena_hippurus", direction = -1) +
  theme_minimal()+
  theme(legend.position="none",
...
pac_15N_raw <- ggplot(PAC_model_data %>% 
  filter(!is.na(δ15N_host)), aes(x = pc1, y = Δ15N, color = Genotype)) +
  geom_point() +
  labs(x = "PC1", 
       y = expression(delta^15*N[H - S]),
      title = " ") +
  scale_color_fish_d(option = "Coryphaena_hippurus", direction = -1) +
  theme_minimal()+
  theme(legend.position="none",
...
#### Linear model:
```{r}
SI_PAC_model_data$Δ15N_T0 <- SI_PAC_model_data$δ15N_T0_host - SI_PAC_model_data$δ15N_T0_symb
...
plot_15N_pc1 <- ggplot() +
  #points
  geom_point(aes(x = pc1, y = Δ15N 
                 #color = Genotype,
                 ), shape = 15, data = SI_PAC_model_data) +
  labs(x = "PC1", 
       y = expression(delta^15*N[H - S]),
      title = " ") +
  scale_color_fish_d(option = "Coryphaena_hippurus", direction = -1) +
  theme_minimal() +
  theme(legend.position="none", #callie you moved this legend for patchwork
        plot.background = element_rect(fill = "white")) +
  #line and ribbon
  geom_line(data = prediction_data, aes(x = pc1, y = fit), color = "#21908c") +  # Add the fitted line
  geom_ribbon(data = prediction_data, aes(x = pc1, ymin = lower, ymax = upper), alpha = 0.2, fill = "#21908c") +
  #annotations
  annotate("text", y = -0.2, x = -4,
         label = paste("p =", round(p_values["pc1"], 6),
                       "\nMarginal R² =", round(marginal_r2, 3),
                       "\nConditional R² =", round(conditional_r2, 3)),
         hjust = 0, size = 3)
...
plot_15N_NN
ggsave("output/pac_15N_NN_mod.png", plot_15N_NN, height = 7, width = 6)
```


##### Looking at H or S
```{r}
linear_nitrogen_host_model_pac <- lmer(δ15N_host ~ pc1 + δ15N_T0_host +
                                  (1|Genotype), data = SI_PAC_model_data)
Anova(linear_nitrogen_host_model_pac)
summary(linear_nitrogen_host_model_pac)
```
```{r}
linear_nitrogen_symb_model_pac <- lmer(δ15N_symb ~ pc1 + δ15N_T0_symb +
                                  (1|Genotype), data = SI_PAC_model_data)
Anova(linear_nitrogen_symb_model_pac)
summary(linear_nitrogen_symb_model_pac)
```
...
pru_15N_raw <- ggplot(PRU_model_data %>% 
  filter(!is.na(Δ15N)), aes(x = pc1, y = Δ15N, 
                            #color = Genotype
                            )) +
  geom_point(shape = 15) +
  labs(x = "PC1", 
...
```{r}
# Combine the plots in one row and collect the legends

toprow <- pac_growth_raw_legend + pac_chl_MOD + pac_13C_raw + plot_15N_pc1 + 
           plot_layout(ncol = 4, guides = "collect") 
bottomrow <- pru_growth_raw_legend + pru_chl_raw + pru_13C_raw + pru_15N_raw + 
           plot_layout(ncol = 4, guides = "collect")

Figure2 <- toprow / bottomrow + 
           plot_annotation(tag_levels = 'A')

#ggsave("output/Figure2.png", plot = Figure2, width = 15, height = 8, dpi = 300)
```

```{r}
legend_pac <- ggpubr::get_legend(pac_growth_raw_legend +
                               guides(shape = guide_legend(order = 2,
                                                           title = "Caging Treatment",
                                                             title.position = "top"),
...
legend_pru <- ggpubr::get_legend(pru_growth_raw_legend +
                               guides(shape = guide_legend(title = "Caging Treatment",
                                                             title.position = "top"),
                                      color = guide_legend(title = "Parent Colony",
                                                             title.position = "top")) +
    theme(legend.position = "right", legend.key.width = unit(0.1, "cm")))


toprow <- plot_grid(pac_growth_raw, pac_chl_MOD, pac_13C_raw, plot_15N_pc1, ncol = 4)
bottomrow <- plot_grid(pru_growth_raw, pru_chl_raw, pru_13C_raw, pru_15N_raw, ncol = 4)
legend <- plot_grid(legend_pac, ncol = 4)

Figure2 <- plot_grid(toprow, bottomrow, legend, ncol = 1, align = "hv")

Figure2
ggsave("output/Figure2.png", plot = Figure2, width = 15, height = 12, dpi = 300)
```
