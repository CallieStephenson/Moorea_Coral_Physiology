...
pac_growth_raw_legend <- ggplot(PAC_model_data_clean, aes(x = pc1, y = Percent_Change, color = Genotype, shape = Cage_Uncaged)) +
  geom_point() +
  labs(
    x = "PC1",
...
### plot raw
```{r}
pru_growth_raw_legend <- ggplot(PRU_model_data, aes(x = pc1, y = Percent_Change, color = Genotype, shape = Cage_Uncaged)) +
  geom_point() +
  scale_shape_manual(
    values = c("A" = 15, "B" = 17, "C" = 16),
    labels = c("A" = "Caged", "B" = "Partially Caged", "C" = "Uncaged")  # Custom shape labels
...
toprow <- plot_grid(pac_growth_raw, pac_chl_MOD, pac_13C_raw, plot_15N_pc1, legend_pac, ncol = 5)
bottomrow <- plot_grid(pru_growth_raw, pru_chl_raw, pru_13C_raw, pru_15N_raw, legend_pru, ncol = 5)

Figure2 <- plot_grid(toprow, bottomrow, ncol = 1, align = "hv")
