...
```{r PAC Discrim}
discr.filename <- here("data", "discr3.csv")
...
#ggsave
ggsave("output/MIXSIARresultspac.png", plot = MixSIAR_PAC,  width = 4, height = 4.5, dpi = 600)
...
MixSIAR_PRU <- ggplot(PRU_MixSIAR, aes(x = Source)) +
  ...
  geom_point(aes(y = Mean * 100), color = "black", size = 2) +  # Add mean points
  scale_fill_manual(values = c("Autotrophy" = "#21908c", 
                               "Heterotrophy" = "#47c16e", 
                               "Seep-derived \n Heterotrophy" = "#fde725"), 
                    name = NULL) + 
  labs(y = "Contribution to Host (%)", x = "") +
...
ggsave("output/MIXSIARresultspru.png", plot = MixSIAR_PRU,  width = 4, height = 4.5, dpi = 600)
```
...
```{r PAC Discrim}
discr.filename <- here("data", "discr2.csv")
discr <- load_discr_data(filename=discr.filename, mix)
```
